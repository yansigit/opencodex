...
사용자 정의 `injectionPrompt`에는 다음 플레이스홀더를 쓸 수 있습니다.
...
| `{{roles}}` | 압축된 사용 중 역할 카탈로그(id, 사용 시점, 모델, 선택적 에포트). 현재 서피스와 700자 예산에 맞게 걸러집니다 |
| `{{nativeDefaultState}}` | `active`, `disabled`, `pending` 또는 `blocked`; 설정된 네이티브 기본값이 권한을 갖는다는 것을 확인하는 값은 `active`뿐입니다 |
...
- **Subagents**: 이름 있는 역할을 편집하고 로스터를 선택·정렬한 뒤 사용자 지정 부모 안내와 전역 자식 지시를 설정합니다.

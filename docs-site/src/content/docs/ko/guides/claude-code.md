...
**2.1.201** 미만이면 `ocx claude`는 경고만 표시하고 계속 실행합니다. `npm install -g @anthropic-ai/claude-code`로 업데이트하고 `ocx doctor`와 `ocx status --json`에서 상태를 확인하세요. 이는 **2.1.129** `/model` 선택기 기능과 별개입니다.

...
構造化出力もこの変換の一部です。`json_object` または `json_schema` を含む
`response_format` は、プロバイダーの `noStructuredOutputModels` によるモデル単位の
オプトアウトを除き、ルーティングされた `openai-chat` モデルに転送されます。リストに
あるモデルではその wire 上の `response_format` を省略し、他のモデルでは変換を維持します。
ルーティングされた Google モデルでは、サポートされるリクエストを Gemini JSON mode
（`responseMimeType` / `responseSchema`）に変換しますが、リクエストにツールがある場合、
選択されたモデルが Claude の場合、またはモデルが画像生成対応の場合はこの変換を
スキップします。Kiro は構造化出力を拒否します。Cursor には構造化出力用のワイヤフィールドがなく、転送前にリクエストを拒否します。

`POST /v1/responses` では、対応するリクエスト フィールドは `text.format` です。ネイティブ
Responses ルートは元の Responses body に保持し、モデルが `openai-chat` プロバイダーに
ルーティングされた場合は `response_format` に変換します。アダプターの動作はその能力に
依存します。実装に応じて機能を転送、スキップ、無視、または拒否するため、表現できない
すべての機能がプロキシによって fail closed になるとは限りません。

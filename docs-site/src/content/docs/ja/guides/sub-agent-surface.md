...
カスタム `injectionPrompt` テキストでは、次のプレースホルダーを使用できます。
...
| `{{roles}}` | 有効なロールの圧縮カタログ（id、使うタイミング、モデル、任意のエフォート）。現在のサーフェスと 700 文字予算でフィルタされます |
| `{{nativeDefaultState}}` | `active`、`disabled`、`pending`、または `blocked`。設定されたネイティブ デフォルトが有効であることを確認できるのは `active` のみです |

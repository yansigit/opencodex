...
**Исключение Antigravity Gemini 3:** если маршрутизируемый провайдер — Google Antigravity (`google-antigravity`)
и wire-модель — Gemini 3.x, opencodex вешает CCA `google_search` (и `url_context`, если в ходе есть
URL `http(s)`) на **тот же** основной запрос вместо sidecar-цикла. Claude-on-CCA, явные бэкенды
`openai` / `anthropic` / `xai` / `exa` и Gemini ниже 3.x с функциональными инструментами Codex
по-прежнему идут через sidecar.

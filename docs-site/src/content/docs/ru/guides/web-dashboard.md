...
В Overview есть панель **Активность за 30 дней** с трендами запросов и токенов за 30 дней. Выберите день на любом спарклайне, чтобы увидеть его итоги, или нажмите **Открыть статистику**, чтобы открыть полную страницу Usage. Если обновление прерывается после загрузки Overview, последние корректные данные остаются на экране и появляется уведомление о переподключении с кнопкой **Повторить**. При ошибке первого подключения показываются подсказка `ocx start` и та же кнопка повторной попытки.

...
| **Subagents** | Выделение до пяти «голых» нативных или маршрутизируемых моделей, редактор до восьми именованных ролей, а также пользовательские подсказки родителю и глобальные инструкции ребёнка. |
...
Макет теперь один и адаптивный, поэтому переключать нечего. На компьютере основная навигация находится в боковой панели, а на узком экране те же ссылки открываются кнопкой **Open menu**. У разделов Dashboard есть собственные адреса: `#dashboard` открывает Overview, а `#dashboard/providers` и `#dashboard/models` — два других раздела. Перезагрузка, закладка и кнопка «Назад» сохраняют выбранный раздел. **Logs** работает так же — `#logs` и `#logs/debug`. Старая закладка `#providers/workspace` теперь ведёт на `#providers`.

В Overview есть панель **30-day activity** с трендами запросов и токенов за 30 дней. Выберите день на любом спарклайне, чтобы увидеть его итоги, или нажмите **View usage**, чтобы открыть полную страницу Usage. Если обновление прерывается после загрузки Overview, последние корректные данные остаются на экране и появляется уведомление о переподключении с кнопкой **Retry**. При ошибке первого подключения показываются подсказка `ocx start` и та же кнопка повторной попытки.

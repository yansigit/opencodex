...
Ниже **2.1.201** `ocx claude` только предупреждает и запускает клиент. Обновите его: `npm install -g @anthropic-ai/claude-code`; статус доступен в `ocx doctor` и `ocx status --json`. Это отдельный порог от `/model` с **2.1.129**.

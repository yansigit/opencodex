...
**Доверие к директивам:** сгенерированные определения подписываются. В теле каждого агента
`ocx-*` есть `<!-- ocx-route: ... -->` (а также `<!-- ocx-effort: ... -->`, если задан effort),
и соответствующая подпись `<!-- ocx-sig: ... -->`, которую OpenCodex создаёт с помощью управляемого
локального ключа подписи. `ocx doctor` сообщает о наличии и правах этого ключа, но никогда не
выводит сам ключ. Прокси проверяет подпись каждого запроса **до любой отправки провайдеру**:
присутствующая, но изменённая, повреждённая или иным образом недействительная подписанная директива
отклоняет запрос с ошибкой `400 invalid_request_error` — он не перенаправляется к другому
провайдеру и не обрабатывается повторно как неподписанный. Неподписанные директивы совместимости
(например, из старого отредактированного вручную определения) учитываются только при точном
совпадении с активной записью ростера, принадлежащей OpenCodex; после неудачной проверки подписи
откат к этому пути ростера не выполняется.

## Анонимный loopback-слушатель (опт-ин)

Если прокси слушает адрес не loopback и требует учётные данные, локальный клиент, который никогда
не получает эти данные, будет отклонён. Именно для этого случая OpenCodex может открыть второй
слушатель на `127.0.0.1`: задайте в конфигурации `unauthenticatedLoopbackListener` (**по
умолчанию выключен**; порт обязателен, должен отличаться от порта прокси и никогда не выбирается
операционной системой автоматически). См. раздел
[Локальные клиенты, которым нельзя передать токен](/reference/configuration/#local-clients-that-cannot-receive-the-token).

Включённый слушатель для Claude Code допускает ровно два маршрута: `POST /v1/messages` и
`POST /v1/messages/count_tokens` (только POST). Любой другой метод или путь — включая `/api/*`
и дашборд — возвращает `404`. Аутентификация на публичном слушателе не меняется: включение
этого слушателя никогда не ослабляет основной слушатель. Маршруты Codex для этого слушателя
...
## Диагностика совместимости клиента

Перед запуском `ocx claude` opencodex проверяет версию Claude Code относительно порога
совместимости **2.1.201**. Проверка даёт одно из пяти состояний с соответствующей рекомендацией:

| Состояние | Значение | Что делать |
| --- | --- | --- |
| `compatible` | Версия не ниже порога | Ничего |
| `outdated` | Версия ниже порога | `npm install -g @anthropic-ai/claude-code` |
| `missing` | Claude Code не установлен | Установите его командой `npm install -g @anthropic-ai/claude-code` |
| `timed-out` | Проверка версии превысила тайм-аут | Повторите; если проблема сохраняется, восстановите или обновите Claude Code |
| `unparseable` | Версию не удалось распознать | Восстановите или обновите Claude Code и повторите |

Проверка носит рекомендательный характер: клиент ниже порога, отсутствующий, не ответивший
вовремя или нераспознанный **никогда не блокирует запуск** — выводится предупреждение, и
`ocx claude` продолжает работу. `ocx doctor` и `ocx status --json` показывают то же состояние
клиента. Этот порог не связан с возможностью шлюзового селектора `/model`, появившейся в
нативной версии **2.1.129**.

### Тест количества токенов (опт-ин, возможны расходы)

Приближение количества токенов для маршрутизируемого пути можно сопоставить с реальными
значениями провайдера командой
`bun run benchmark:claude-tokens -- --provider <provider> --model <model> --confirm-live-provider-charges [--json]`.
Сама команда **является согласием**: без `--confirm-live-provider-charges` она только проверяет
аргументы и ничего не отправляет. После подтверждения она отправляет реальные запросы и **может
повлечь расходы провайдера**. Никогда не автоматизируйте и не запускайте её без присмотра —
выполняйте намеренно, контролируя аккаунт.

Что делает команда:

- Работает только с парами провайдер/модель Anthropic-адаптера (провайдер должен быть
  аутентифицирован ключом и перечислять модель), чтобы upstream возвращал достоверное
  значение `input_tokens`.
- Отправляет детерминированный санитизированный набор фикстур — пользовательский текст не
  читается и не встраивается.
- Отправляет фикстуры по одной; ошибки типизируются и никогда не повторяются, параллелизм и
  fallback не используются.
- Формирует закрытый непостоянный отчёт: только идентификаторы фикстур, дайджесты, состояния,
  метрики, вид провайдера и id модели. Тела запросов, учётные данные и идентификаторы аккаунтов
  никогда не записываются.
- Применяет допуск для каждой фикстуры `max(32 tokens, 20%)` и проходит только при абсолютной
  средневзвешенной ошибке не более 10%.

Поведение маршрутизируемого `/v1/messages/count_tokens` самим тестом не меняется: для
маршрутизируемых моделей он остаётся локальным, а к Anthropic передаётся только при наличии
нативных учётных данных `sk-ant-`.

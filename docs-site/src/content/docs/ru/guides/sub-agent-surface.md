...
Пользовательский `injectionPrompt` может использовать эти placeholder'ы:
...
| `{{roles}}` | Компактный каталог включённых ролей (id, когда использовать, модель, необязательный effort), отфильтрованный под текущую поверхность и бюджет 700 символов |
| `{{nativeDefaultState}}` | `active`, `disabled`, `pending` или `blocked`; только `active` подтверждает полномочность настроенного native default |
...
- **Dashboard** → **Sub-agent delegation**: задайте модель/effort для guidance и opt-in для
  native-default.
- **Subagents**: редактируйте именованные роли, выбирайте и упорядочивайте ростер, настраивайте пользовательские подсказки родителю и глобальные инструкции ребёнка.

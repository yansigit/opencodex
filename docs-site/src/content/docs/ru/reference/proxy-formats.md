...
Структурированный вывод также является частью этого преобразования. `response_format` с типом
`json_object` или `json_schema` пересылается маршрутизируемым моделям `openai-chat` с учётом
помодельного opt-out через `noStructuredOutputModels`: перечисленные модели пропускают
`response_format` на этом wire, остальные сохраняют преобразование. Маршрутизируемые модели
Google преобразуют поддерживаемые запросы в Gemini JSON mode
(`responseMimeType` / `responseSchema`), но пропускают это преобразование, если в запросе есть
инструменты, выбранная модель — Claude или модель поддерживает генерацию изображений. Kiro
отклоняет структурированный вывод. У Cursor нет полевого представления структурированного вывода на проводе, и запрос отклоняется до транспорта.

В `POST /v1/responses` соответствующее поле запроса — `text.format`: native Responses-маршруты
сохраняют его в исходном Responses body, а при маршрутизации модели к провайдеру `openai-chat`
преобразуют его в `response_format`. Поведение адаптера зависит от его возможностей: в
соответствии с реализацией функция может быть переслана, пропущена, проигнорирована или
отклонена; прокси не гарантирует fail-closed для каждой функции, которую нельзя представить.

...
| `injectionPrompt?` | `string` | — | Remplace le corps des consignes v2 intégrées. Accepte `{{model}}`, `{{effort}}`, `{{roster}}`, `{{fallback}}` et `{{nativeDefaultState}}`. Ce dernier vaut `active`, `disabled`, `pending` ou `blocked` ; seul `active` confirme l’autorité de la valeur native par défaut configurée. La présence d’un `injectionModel` suffit pour produire le prompt personnalisé. |

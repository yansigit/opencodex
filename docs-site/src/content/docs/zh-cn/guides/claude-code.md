...
**指令信任：**生成的定义带有签名。每个 `ocx-*` 代理正文都包含 `<!-- ocx-route: ... -->`（配置了努力级别时还包含 `<!-- ocx-effort: ... -->`）以及匹配的 `<!-- ocx-sig: ... -->` 签名；该签名由 OpenCodex 使用其自动管理的本地签名密钥创建。`ocx doctor` 会报告该密钥是否存在及其权限，但绝不会打印密钥本身。代理会在每次请求中、**任何提供方派发之前**验证签名：已存在但被修改、损坏或其他无效的签名指令都会以 `400 invalid_request_error` 拒绝请求；绝不会改路由到其他提供方，也绝不会按无签名处理而重新处理。无签名的定义仅在其指令与活动的 OpenCodex 所有名册条目完全匹配时才会采用；签名校验失败绝不会回退到该名册路径。

...
`POST /v1/messages` 和 `POST /v1/messages/count_tokens` 也是在选择启用的 `unauthenticatedLoopbackListener` 上允许的唯二 Claude 路由（服务器级密钥；必须指定端口，且必须不同于代理端口）。公共监听器的认证不受该监听器影响。参见[无法接收令牌的本地客户端](#local-clients-that-cannot-receive-the-token)。
## 影子调用

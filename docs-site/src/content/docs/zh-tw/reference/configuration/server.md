...
`POST /v1/alpha/search`（Codex 原生網頁搜尋中繼）、`GET /v1/models`、`POST /v1/messages`、
`POST /v1/messages/count_tokens`，以及獨立語音 WebSocket upgrade。兩個 Messages 路由支援使用
Anthropic 協定的本機 Claude Code 用戶端。其他一切，包括 `/api/*` 與儀表板，都會回傳 `404`。

...
describe("agent maintenance workflow", () => {
  ...
  it("reconciles exact-head reviews and enforces the repair ceiling", () => {
    ...
    assert.match(workflow, /currentBaseSha/);
    assert.match(workflow, /controllerMerge/);
    assert.match(workflow, /sessionStatus/);
    assert.match(workflow, /liveAutonomous = autonomousMergeEvidence/);
    assert.match(workflow, /pulls\.merge/);
    assert.match(workflow, /sha: live\.head\.sha/);
    assert.doesNotMatch(workflow, /rawEvidence\(live\)|controllerEvidence\(live/);
    assert.match(workflow, /changedFileListComplete\(live\.changed_files, liveFiles\)/);
    assert.match(workflow, /requiredNames: baselineNames/);
    assert.match(workflow, /getBranch\(\{ owner, repo, branch: "dev" \}\)/);
    assert.match(workflow, /basehead: `\$\{liveBaseSha\}\.\.\.\$\{live\.head\.sha\}`/);
    assert.match(workflow, /liveComparison\.behind_by !== 0/);
    assert.match(workflow, /pulls\.listReviews/);
    assert.match(workflow, /CHANGES_REQUESTED/);
    assert.match(workflow, /liveUnresolvedThreads !== 0/);
    ...
    assert.match(workflow, /\["ci", "enforce-target", "hygiene", "mergeable"\]/);

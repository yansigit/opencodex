...
import { OAuthTransportError, oauthFetch } from "./transport";
...
export class OrcaRouterOAuthFlow extends OAuthCallbackFlow {
  ...
  async exchangeToken(code: string, _state: string, _redirectUri: string): Promise<OAuthCredentials> {
    ...
    // The shared OAuth transport enforces the bounded deadline, capped response
    // size, and redirect refusal. Plain HTTP is only requested for a self-hosted
    // loopback origin already validated by normalizeOrcaRouterBaseUrl; the
    // transport re-checks the loopback constraint independently.
    const allowLoopbackHttp = !this.#authBaseUrl.startsWith("https:");
    let response: Response;
    try {
      response = await oauthFetch(new URL("/api/v1/auth/keys", this.#authBaseUrl), {
        method: "POST",

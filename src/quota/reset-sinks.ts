...
async function deliverWebhook(
  ...
  let publicPinned: { address: string; family: number } | undefined;
  if (!config.allowPrivateNetwork) {
    try {
      const resolved = await (dependencies.resolveAddresses ?? resolvePublicAddresses)(url, {
        context: "quota reset webhook URL",
      });
      publicPinned = resolved.addresses.find(address => address.family === 4)
        ?? resolved.addresses[0];
      if (!publicPinned) return { sink: "webhook", ok: false, reason: "blocked-destination" };
    } catch {
  ...
  try {
    ...
    if (config.allowPrivateNetwork) {
      // The explicit private-network opt-in preserves ordinary fetch semantics so local
      // receivers and split-horizon names remain usable. Redirects still stay disabled.
      ...
      // Validation and connection must use the SAME DNS answer. A validate-then-fetch pair
      // resolves twice, so an attacker-controlled hostname can answer public first and rebind
      // to loopback or metadata for the actual POST.
      response = await (dependencies.pinnedPost ?? pinnedHttpPost)(
        url,
        publicPinned!,
        json,
        timeout.signal,

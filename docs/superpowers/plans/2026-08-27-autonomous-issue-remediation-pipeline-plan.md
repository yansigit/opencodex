...
- Core path (`src/router.ts`, `src/server/lifecycle.ts`, `src/server/responses/core.ts`) must NOT import `src/lab/` directly or transitively (`tests/lab/core-lab-boundary.test.ts` must pass).
...
- `tests/usage/telemetry-fingerprint.test.ts`
- `tests/usage/telemetry-ledger.test.ts`
...
- TDD: write tests in `tests/usage/telemetry-fingerprint.test.ts` and `tests/usage/telemetry-ledger.test.ts` verifying hashing stability and rolling window threshold counting.
...
- `tests/usage/telemetry-hook.test.ts`
...
- TDD: write tests in `tests/usage/telemetry-hook.test.ts` simulating stream drops and verifying telemetry capture.
...
- `src/cli/index.ts`
- `tests/usage/telemetry-dispatcher.test.ts`
...
- TDD: write tests in `tests/usage/telemetry-dispatcher.test.ts` mocking command runner and verifying payload format, remote search, and authorization gating.

...
**Step 1 (RED):** Write a unit test in `tests/providers/command-code-provider.test.ts` with two requests having identical tools in reversed/shuffled order. Assert that `buildRequest` produces identical `params.tools` order and identical `params.system` text. Watch test fail.
...
**Step 3 (VERIFY):** Run `bun test tests/providers/command-code-provider.test.ts` to verify.
...
**Step 1 (RED):** In `tests/providers/command-code-provider.test.ts`, test that `commandCodeSessionId` produces stable UUIDs across turns when:
...
**Step 3 (VERIFY):** Run `bun test tests/providers/command-code-provider.test.ts` and relevant server tests.
...
**Files to change:**
- `tests/providers/command-code-workspace-cache.test.ts`
- `src/adapters/command-code.ts`
**Step 1 (RED):** Write a unit test in `tests/providers/command-code-workspace-cache.test.ts` verifying that `structure` is always sorted alphabetically, and that cached config per session preserves its initial date across midnight.
...
**Step 3 (VERIFY):** Run `bun test tests/providers/command-code-workspace-cache.test.ts`.
...
**Step 1:** Run `bun test tests/command-code-*.test.ts tests/providers/commandcode-provider.test.ts`.

...
test("management routes cannot reach global config writers through any runtime helper", async () => {
  ...
  const allowed = new Set([
    ...
    // Model discovery owns this locked, registration-fenced initial-selection commit;
    // management can trigger discovery but cannot supply a stale whole-config writer.
    "providers/initial-model-selection-runtime.ts: mutatePersistedConfig",
    ...
    // The settings route can request an immediate refresh after enabling a window. The
    // worker persists only completed reset markers through the locked mutation boundary.
    "codex/quota-auto-refresh.ts: mutatePersistedConfig",
  ]);
  const api = new API({ cwd: join(SRC, "..") });
  try {

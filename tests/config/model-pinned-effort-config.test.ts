...
import {
  deleteConfigTopLevelKey, getConfigPath, getDefaultConfig, loadConfig, mutatePersistedConfig, readConfigDiagnostics,
...
import { ManagementRequest, isolatedDiskManagementPersistence } from "../helpers/management-auth";
...
function context(config: OcxConfig, path: string, method: string, body?: unknown): ManagementContext {
  ...
  return {
    ...
    deps: { ...isolatedDiskManagementPersistence(), clearThreadAccountMap: () => {}, clearProviderQuotaCache: () => {} },
  ...
  test("GET returns provider pins and canonical OpenAI PATCH/POST round-trip them", async () => {
    ...
    try {
      mutatePersistedConfig(fresh => {
        fresh.providers.openai = providerConfigSeed(getProviderRegistryEntry("openai")!);
  ...
  test("PATCH and POST save failures restore exact provider ownership and pending deletion metadata", async () => {
    ...
    try {
      for (const method of ["PATCH", "POST"]) {
        ...
        ctx.deps.mutatePersistedConfig = mutate => {
          const candidate = loadConfig();
          mutate(candidate);
          deleteConfigTopLevelKey(candidate, "modelPinnedEfforts");
          // Restore the value but leave the injected deletion intent pending.
          candidate.modelPinnedEfforts = beforeLive.modelPinnedEfforts;
          candidate.configRebaseProvenance = { version: 1, deletedTopLevelKeys: ["effortCap"] };

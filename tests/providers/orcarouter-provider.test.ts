...
describe("OrcaRouter dual authentication", () => {
  ...
  test("exchanges through the shared bounded transport on a loopback self-hosted origin", async () => {
    let requestUrl = "";
    globalThis.fetch = (async (input: string | URL | Request, init?: RequestInit) => {
      requestUrl = String(input);
      expect(init?.redirect).toBe("manual");
      expect(init?.signal).toBeInstanceOf(AbortSignal);
      return Response.json({ key: "sk-orca-selfhost-test", user_id: 7 });
    }) as typeof fetch;
    const flow = new OrcaRouterOAuthFlow({}, { baseUrl: "http://127.0.0.1:9999/v1" });
    await flow.generateAuthUrl("state", "http://127.0.0.1:51733/callback");
    const credential = await flow.exchangeToken("code", "state", "ignored");
    expect(requestUrl).toBe("http://127.0.0.1:9999/api/v1/auth/keys");
    expect(credential).toMatchObject({ access: "sk-orca-selfhost-test", accountId: "7" });
  });

  test("caps an oversized key-exchange response at the shared 1 MiB OAuth boundary", async () => {
    globalThis.fetch = (async () =>
      new Response(new Uint8Array(1024 * 1024 + 1), { status: 200 })) as typeof fetch;
    const flow = new OrcaRouterOAuthFlow({});
    await flow.generateAuthUrl("state", "http://127.0.0.1:51733/callback");
    await expect(flow.exchangeToken("code", "state", "ignored"))
      .rejects.toThrow("response exceeded 1 MiB");
  });

...
function expectResultOutput(bytes: Uint8Array, modelId: string, output: string, isError = false, toolName = "exec") {
  const { roots, steps } = decodedReplay(bytes);
  expect(roots).toHaveLength(1);
  const completion = isError ? "" : "\n[completed: this tool invocation already ran successfully; do not repeat it]";
  expect(roots[0]).toContain(`\noutput:\n${output}`);
  expect(roots[0].includes("\nis_error: true\n")).toBe(isError);
  expect(roots[0].endsWith(output + completion)).toBe(true);
  ...
  if (modelId === "composer-2.5") {
    ...
    if (toolName === "exec") {
      expect(steps[0].startsWith(isError ? "Tool error for exec" : "Tool output for exec")).toBe(true);
      expect(steps[0]).toContain(`is_error: ${isError}):`);
    } else {
      expect(steps[0].startsWith(isError ? "[Tool Error]" : "[Tool Result]")).toBe(true);
      expect(steps[0].includes("\nis_error: true\n")).toBe(isError);
    }
    expect(steps[0].endsWith(`\n${output}${completion}`)).toBe(true);
...
describe("Cursor host failure provenance and successful-output regression", () => {
  for (const modelId of ["composer-2.5", "grok-4.6"]) {
    ...
    test(`${modelId}: node_repl keeps its legacy error guidance on replay`, () => {
      ...
      expectResultOutput(requestWith(failure, {}, { modelId, tools: codeModeTools }), modelId, output, true, "node_repl");
      expectResultOutput(requestWith(output, { isError: true }, { modelId, tools: codeModeTools }), modelId, output, true, "node_repl");
    });

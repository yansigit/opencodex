...
test("uses grouped summary windows when the daily RPC is unavailable", async () => {
  ...
  const result = await fetchAntigravityLiveQuota({
    ...
    fetchImpl: async (input: RequestInfo | URL) => {
      const url = String(input);
      requested.push(url);
      if (url.endsWith(":retrieveUserQuota") && !url.endsWith(":retrieveUserQuotaSummary")) {
        return jsonResponse({}, 404);
      }
      if (url.endsWith(":retrieveUserQuotaSummary")) {
        return jsonResponse({
          groups: [
            {
              ...
              buckets: [
                { window: "5h", remaining: { remainingPercentage: 0.5 } },
            ...
            },
            {
              ...
              buckets: [
                ...
                { window: "weekly", remainingFraction: 0.7 },
              ],
            },
          ],
        });
  ...
  expect(result?.customWindows).toEqual([
    { label: "Gem", percent: 99.5 },
    { label: "Gem (Weekly)", percent: 20 },
    { label: "Cla", percent: 10 },
    { label: "Cla (Weekly)", percent: 30 },
  ]);
  expect(requested.filter(url => url.startsWith(PROD_HOST))).toEqual([]);
});

test("retries production for a partial grouped summary and keeps canonical window order", async () => {
  const requested: string[] = [];
  const result = await fetchAntigravityLiveQuota({
    accessToken: TOKEN,
    projectId: PROJECT,
    ...
    fetchImpl: async (input: RequestInfo | URL) => {
      ...
      requested.push(url);
      if (url.endsWith(":retrieveUserQuota") && !url.endsWith(":retrieveUserQuotaSummary")) {
        return url.startsWith(DAILY_HOST) ? jsonResponse({}, 404) : liveGeminiQuota();
      }
      if (url.endsWith(":retrieveUserQuotaSummary")) {
        return jsonResponse({
          groups: [{
...
describe("Antigravity live quota", () => {
  test("does not run live or catalog fallbacks after a terminal pinned summary response", async () => {
    setAntigravityAccountQuotaTransportForTests({
      resolveAddresses: async () => ({
        hostname: "daily-cloudcode-pa.googleapis.com",
        addresses: [{ address: "142.250.0.1", family: 4 }],
      ...
      pinnedPost: async () => jsonResponse({}, 429),
    });
    const requested: string[] = [];
  ...
  test("Antigravity quota never sends OAuth credentials to a configured non-canonical base URL", async () => {

...
import { chmodSync, copyFileSync, linkSync, mkdirSync, mkdtempSync, readFileSync, realpathSync, rmSync, statSync, writeFileSync } from "node:fs";
...
function windowsChildProcessIdentity(parentPid: number): WindowsProcessIdentity | null {
  ...
  for (const value of parsed) {
    // A Windows console host or another launcher helper can briefly share the
    // parent. The proof argument is generated by bin/ocx.mjs and belongs only
    // to the Bun process that runs the real CLI.
    if (
      Number.isSafeInteger(value.ProcessId)
      ...
      && value.CommandLine.includes(NODE_LAUNCH_PROOF_PREFIX)
    ) {
      return {
        pid: value.ProcessId!,
        parentPid: value.ParentProcessId,
...
function sameWindowsPath(actual: string, expected: string): boolean {
  if (canonicalWindowsPath(actual) === canonicalWindowsPath(expected)) return true;
  try {
    // CIM may report a different hard-link spelling from the path passed to
    // CreateProcess. File identity preserves the exact-executable assertion.
    const actualIdentity = statSync(actual, { bigint: true });
    const expectedIdentity = statSync(expected, { bigint: true });
    return actualIdentity.ino !== 0n
...
test("Windows executable comparison recognizes hard-link aliases by file identity", () => {
  const root = mkdtempSync(join(tmpdir(), "ocx-launcher-file-identity-"));
  try {
    const executable = join(root, "bun.exe");
    const alias = join(root, "bun-alias.exe");
    const other = join(root, "other.exe");
    writeFileSync(executable, "bun image", "utf8");
    linkSync(executable, alias);
    writeFileSync(other, "bun image", "utf8");

    expect(sameWindowsPath(alias, executable)).toBe(true);
    expect(sameWindowsPath(other, executable)).toBe(false);
  } finally {
    removeTree(root);
  }
});

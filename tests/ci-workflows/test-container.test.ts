...
test("image and ignore policy freeze dependencies and exclude host state", () => {
  ...
  const ignored = readFileSync(join(ROOT, "Containerfile.test.dockerignore"), "utf8").split("\n");
...
test("ignore policies exclude nested credentials and state from allowed subtrees", () => {
  const contexts = [
    { file: ".dockerignore", fixtureIncluded: false },
    { file: "Containerfile.test.dockerignore", fixtureIncluded: true },
  ];
  ...
  for (const { file, fixtureIncluded } of contexts) {
    ...
    for (const path of sensitivePaths) expect(buildContextIncludes(rules, path), `${file}: ${path}`).toBe(false);
    expect(buildContextIncludes(rules, "docker/bootstrap-token.ts"), file).toBe(true);
    expect(buildContextIncludes(rules, "gui/src/ui.tsx"), file).toBe(true);
    expect(buildContextIncludes(rules, "tests/fixtures/network-tls-test-cert.pem"), file).toBe(fixtureIncluded);
    expect(buildContextIncludes(rules, "tests/fixtures/network-tls-test-key.pem"), file).toBe(fixtureIncluded);
  }
});

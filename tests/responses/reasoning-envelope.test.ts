...
describe("reasoning allocation admission", () => {
  ...
  test.each(["thinking", "redacted_thinking", "owned"])("handler maps %s admission failure to 413 without dispatch and disposes its budget", async type => {
    ...
    try {
      ...
      // The fork retains the source envelope before translation, but never the failed translated body.
  ...
  test("source envelope admission rejects before allocating its clone", async () => {
    const { captureClaudeSourceEnvelope } = await import("../../src/server/claude-messages");
    const budget = budgets.createTranslatorBudget({ maxTurnBytes: 64 });
    const clone = spyOn(globalThis, "structuredClone");
    try {
      expect(() => captureClaudeSourceEnvelope(new Request("http://localhost/v1/messages"), {
        messages: [{ role: "user", content: "x".repeat(200) }],
      }, budget)).toThrow(TranslatorBudgetExceededError);
      expect(clone).not.toHaveBeenCalled();
      expect(budget.snapshot().currentBytes).toBe(0);
    } finally { clone.mockRestore(); budget.dispose(); }
  });
  ...
  test("final request-copy admission returns 413 before serialization and disposes the budget", async () => {
    ...
    const beforeBytes = budgets.translatorObservedBufferSnapshot().currentBytes;
    const beforeCount = budgets.translatorLiveBudgetCountForTests();
    const budget = budgets.createTranslatorBudget({ maxTurnBytes: 1024 });
    const reserve = spyOn(budget, "reserveTransient");
    const charge = spyOn(budget, "chargeRetained");
    const factory = spyOn(budgets, "createTranslatorBudget").mockReturnValue(budget);
    const stringify = spyOn(JSON, "stringify");
    ...
    try {
      ...
      expect(response.status).toBe(413);
      expect(await response.json()).toMatchObject({ type: "error", error: { type: "request_too_large", code: "translation_buffer_limit" } });
      expect(charge.mock.calls.filter(([, scope]) => scope.kind === "request_copies")).toHaveLength(2);
  ...
  test("successful Request construction retains source and translated bodies after releasing temporary copies", async () => {
    const { handleClaudeMessages } = await import("../../src/server/claude-messages");
    const request = new Request("http://localhost/v1/messages", {
      method: "POST", headers: { "content-type": "application/json" },
    ...
    try {
      ...
      expect(copies[0]!.bytes).toBe(Buffer.byteLength(JSON.stringify({ model: "fixture/model", messages: [{ role: "user", content: "hello" }] })));
      expect(copies[2]!.bytes).toBe(expected);
      expect(copies[2]!.before).toBe(copies[1]!.after);

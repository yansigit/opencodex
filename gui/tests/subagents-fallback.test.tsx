...
type CachedSubagents = {
  available: string[];
  chosen: string[];
  fallback?: string[];
  pollMs?: number;
  fallbackAvailable?: string[];
  catalogState?: { state: "fresh" | "stale" | "not_running" | "unknown" };
};
...
test.each(failedFallbackReads)("cold roster survives fallback $name and recovers through retry", async ({ response }) => {
  ...
  const rosterSaveRow = container.querySelector(".swi-featured-list")?.closest("section")?.querySelector(".swi-save-row");
...
test("independent roster Save never caches an unsaved fallback draft", async () => {
  ...
  expect(putBodies(ROSTER_PATH)).toEqual([{ models: ["a-1", "a-3"] }]);
  expect(putBodies()).toEqual([]);
  expect(cached()).toEqual({ available, fallbackAvailable: available, chosen: ["a-1", "a-3"], fallback: ["a-2"], pollMs: 45_000, catalogState: { state: "unknown" } });
  expectOrder(["a-2", "a-3"]);
  expect(pollInput().value).toBe("90000");

  // Saving the fallback afterward must retain the committed roster and its catalog status.
  await click(saveButton());
  expect(putBodies()).toEqual([{ models: ["a-2", "a-3"], pollMs: 90_000 }]);
  expect(cached()).toEqual({ available, fallbackAvailable: available, chosen: ["a-1", "a-3"], fallback: ["a-2", "a-3"], pollMs: 90_000, catalogState: { state: "unknown" } });
});
...
test("a legacy cache keeps fallback disabled through GET failure, roster Save, and remount", async () => {
  ...
  if (!rosterSaveRow) throw new Error("Roster Save row not found");
  await click(saveButton(rosterSaveRow));
  const savedRosterCache = { available, chosen: ["a-1", "a-3"], catalogState: { state: "unknown" } as const };
  expect(putBodies(ROSTER_PATH)).toEqual([{ models: ["a-1", "a-3"] }]);
  ...
  try {
    ...
    expect(requests.filter(request => request.path === FALLBACK_PATH && request.method === "GET")).toHaveLength(getsBefore + 1);
    expect(pendingFallbackResponse).toBeNull();
    // The remount re-seeds the cache from the fresh pending GET and drops catalogState.
    await assertBlocked({ available, chosen: ["a-1", "a-3"] });
  } finally {
    await act(async () => {

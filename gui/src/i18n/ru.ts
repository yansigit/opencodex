...
"codexSet.dialog.notRendered": "На прочитанном нами шаге этот слой ничего не отправил. Разделы пересылаются только при изменении, поэтому в одной выборке слой может отсутствовать.",
  ...
  "sub.routedDelegationBridgeHint": "Экспериментальная функция. Codex может по-прежнему показывать исходную модель; маршрутизированные запросы, контекст репозитория, результаты инструментов, доступность, контекст, поведение, выставление счетов и конфиденциальность зависят от выбранного провайдера. Инструменты выбирает модель.",

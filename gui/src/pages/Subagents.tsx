...
type CachedSubagents = { available: string[]; chosen: string[]; catalogState?: { state?: CatalogState }; fallback?: string[]; pollMs?: number; fallbackAvailable?: string[] };
...
export default function Subagents({ apiBase }: { apiBase: string }) {
  ...
  const [ultraMode, setUltraMode] = useState<UltraModeState>({
    enabled: false, hintText: null, recommendation: null, multiAgentV2Enabled: false, multiAgentMode: "default",
  });
  ...
  const loadSubagents = useCallback(async (signal?: AbortSignal): Promise<CachedSubagents> => {
    ...
    const response = await readJsonOrThrow<{ available?: string[]; chosen?: string[]; catalogState?: { state?: CatalogState } }>(rosterRes, t("sub.loadFail"));
  ...
  const save = async () => {
    ...
    try {
      ...
      const next = { ...committed.current, available, chosen: applied, catalogState: { state: catalogState } };
      committed.current = next;
  ...
  const saveFallback = async () => {
    ...
    try {
      ...
      const next = { ...committed.current, available, chosen: committed.current?.chosen ?? [], ...fallbackSnapshot.current };
      committed.current = next;
      writeSessionListCache(cacheKey, next);
      setOk(true);

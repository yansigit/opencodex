...
function buildHeatmap(days: UsageDay[], locale: Locale, customWindow = false): { weeks: HeatmapCell[][]; months: { label: string; col: number }[]; buckets: number[] } {
  ...
  if (customWindow) {
    ...
    for (const day of days) {
      ...
      if (monthIndex !== previousMonth && weeks.length - lastMonthCol >= 4) {
        months.push({ label: monthName.format(calendar), col: weeks.length });
...
export default function Usage({ apiBase, connected = false, apiKeyId }: { apiBase: string; connected?: boolean; apiKeyId?: string }) {
  ...
  const heatmap = useMemo(() => buildHeatmap(data?.days ?? [], locale, !!customWindow), [data?.days, locale, customWindow]);

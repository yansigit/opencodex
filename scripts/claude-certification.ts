...
import { aliasForRoute } from "../src/claude/alias";
...
function config(baseUrl: string): OcxConfig {
  return { port: 0, hostname: "127.0.0.1", defaultProvider: "claude-cert", providers: { "claude-cert": { adapter: "anthropic", baseUrl, authMode: "key", apiKey: "hermetic-cert-key", allowPrivateNetwork: true, liveModels: true, models: [MODEL], retry: { maxAttempts: 1 } } } } as OcxConfig;
}
...
export async function runHermetic(cli = process.env.CLAUDE_BIN?.trim() || "claude"): Promise<CertificationReport> {
  ...
  try {
    ...
    const env = sanitizedChildEnv(process.env, dirs); env.ANTHROPIC_BASE_URL = new URL("/v1", proxy.url).toString(); env.ANTHROPIC_MODEL = aliasForRoute("claude-cert", MODEL) ?? MODEL;
    let result; try { result = await command(cli, ["-p", "Read the marker file and print its contents.", "--model", env.ANTHROPIC_MODEL, "--allowedTools", "Read", "--output-format", "text"], root, env); } catch (error) { return { mode: "hermetic", status: "skipped", cliPresent: false, streaming: false, toolContinuation: false, requests: 0, reason: error instanceof Error ? error.message : "Claude Code CLI unavailable" }; }
